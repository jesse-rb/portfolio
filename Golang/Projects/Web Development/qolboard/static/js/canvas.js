...
function Canvas(toggleNotifications, notificationsBar, joinNotifIncoming, joinNotifOutgoing, membersContainer, displayName, setDisplayName, codeInput, joinButton, codeDisplay,
    ...
    _newGroup.addEventListener('click', function(e) {
        _groups.push(new Group(_me, _groupsContainer, _canvasContainer))
    }, false);
    ...
    _canvas.addEventListener('mouseup', function (e) {
        if (_mouseDown && _activeTool == 'draw') {
            if (_activeGroup != null) {
                _activeGroup.AddPiece(_pieces[_pieceInProgressIndex]);
            }
    ...
    this.SetActiveGroup = function(group) {
        // Show through ui that this group is active
        for (g of _groups) {
            g.Deselect();
        }
        group.Select();
    ...
    this.GetBackgroundColor = function() {
        return _canvasBackgroundColorPicker.value;
    }
    ...
    this.CleanAroundPiece = function(p) {
        for (piece of _pieces) {
            if (p.Collide(piece) && piece != p) {
                piece.Show();
            }
        }
    }
