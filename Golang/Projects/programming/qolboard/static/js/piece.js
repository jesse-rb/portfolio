...
function Piece(canvas, parentElem, startX, startY, ctx, miniMapCtx, color, size, shadowColor, shadowBlur) {
    ...
    let hideSettings = function(ctx, i) {
        ctx.beginPath();
        ctx.moveTo(_xPath[i], _yPath[i]);
        ctx.lineTo(_xPath[i+1], _yPath[i+1]);
        ctx.lineCap = 'round';
        ctx.lineJoin = 'bevel';
        ctx.strokeStyle = _c.GetBackgroundColor();
        ctx.fillStyle = _c.GetBackgroundColor();
        ctx.lineWidth = (_size*1.5)+(_shadowBlur*3);
        ctx.shadowColor = _c.GetBackgroundColor();
        ctx.shadowBlur = 0;
        ctx.fill();
        ctx.stroke();
        ctx.closePath();
    ...
    this.InBoundCollide = function(x, y) {
        for (let i = 0; i < _xPath.length; i++) {
            if (x > _xPath[i]-((_size*2)+(_shadowBlur*3.5)) &&
                x < _xPath[i]+((_size*2)+(_shadowBlur*3.5)) &&
                y > _yPath[i]-((_size*2)+(_shadowBlur*3.5)) &&
                y < _yPath[i]+((_size*2)+(_shadowBlur*3.5))) {
                return true;
            }
        }
    ...
    // Move piece
    this.MoveLeft = function(moveStr) {
        for (let i = 0; i < _xPath.length; i++) {
            _xPath[i]-=moveStr; // Move each point right
        }
    }
    this.MoveRight = function(moveStr) {
        for (let i = 0; i < _xPath.length; i++) {
            _xPath[i]+=moveStr; // Move each point right
        }
    }
    this.MoveUp = function(moveStr) {
        for (let i = 0; i < _xPath.length; i++) {
            _yPath[i]-=moveStr; // Move each point right
        }
    }
    this.MoveDown = function(moveStr) {
        for (let i = 0; i < _xPath.length; i++) {
            _yPath[i]+=moveStr; // Move each point right
        }
    }
    ...
    this.ScaleUp = function(scaleFactor) {
        ...
        let diffY = _yPath[0];
        for (let i = 0; i < _xPath.length; i++) {
            _xPath[i]*=scaleFactor; // Scale
        }
        for (let i = 0; i < _yPath.length; i++) {
            _yPath[i]*=scaleFactor; // Scale
        }
    ...
    this.ScaleDown = function(scaleFactor) {
        ...
        let diffY = _yPath[0];
        for (let i = 0; i < _xPath.length; i++) {
            _xPath[i]*=1/scaleFactor; // Scale
        }
        for (let i = 0; i < _yPath.length; i++) {
            _yPath[i]*=1/scaleFactor; // Scale
        }
        diffX = diffX-_xPath[0];
        diffY = diffY-_yPath[0];
        _me.MoveDown(diffY);
    ...
    this.SetShadowBlur = function(blur) {
        _shadowBlur = blur;
    }
